...
<body>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-3 sm:px-4">
      <div className="w-full max-w-[820px] max-h-[92vh] overflow-y-auto rounded-[14px] sm:rounded-[20px] bg-white shadow-2xl relative">
        <button
          onClick={onClose}
          className="absolute right-4 sm:right-5 top-4 sm:top-5 text-[#6b7280] hover:text-[#111827] z-10"
        >
          <X size={22} />
        </button>
        ...
        <div className="px-4 sm:px-8 py-6 sm:py-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              ...
              <p className="text-[16px] sm:text-[20px] font-semibold text-[#6B21A8] mt-2">
                {bill.bill_number || "-"}
              </p>
            </div>
            <div className="sm:text-right">
              <span
                className="inline-flex px-3 sm:px-4 py-1 rounded-full font-semibold text-[11px] sm:text-[13px] uppercase"
                style={{ background: statusStyle.bg, color: statusStyle.color }}
              >
                {bill.status}
              </span>
              <div className="mt-3 text-[12px] sm:text-[13px] text-[#6b7280]">
          ...
          {/* Vendor + Meta */}
          <div className="mt-6 sm:mt-8 border-t border-[#e5e7eb] pt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            <div>
              <h3 className="text-[10px] sm:text-[11px] font-bold text-[#9ca3af] uppercase tracking-wide mb-2">
                Vendor
              </h3>
            ...
            <div>
              ...
              <div className="text-[12px] sm:text-[13px] text-[#374151] space-y-1">
                <div>
                  Order #:{" "}
                ...
                {bill.subject && (
                  <div>
                    Subject:{" "}
                    <span className="font-medium">{bill.subject}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="mt-6 sm:mt-8">
            <h3 className="text-[14px] sm:text-[15px] font-semibold text-[#111827] mb-3">
              Items
            </h3>
                ...
                <tbody>
                  {items.length === 0 ? (
                    <tr>
                      <td
                        colSpan={5}
                        className="px-4 py-6 text-center text-[12px] sm:text-[13px] text-[#9ca3af]"
                      >
                        No items
                      </td>
                    </tr>
                    ...
                    items.map((it, i) => (
                      ...
                      >
                        ...
                        <td className="px-3 sm:px-4 py-3 text-[12px] sm:text-[13px] text-[#6b7280] hidden sm:table-cell">
                          {it.account || "-"}
                        </td>
                        <td className="px-3 sm:px-4 py-3 text-[12px] sm:text-[13px] text-center">
                          {Number(it.quantity || 0)}
                        </td>
                        <td className="px-3 sm:px-4 py-3 text-[12px] sm:text-[13px] text-right text-[#6b7280]">
                          {fmt(it.rate)}
                        </td>
                        <td className="px-3 sm:px-4 py-3 text-[13px] sm:text-[14px] text-right font-semibold">
                          {fmt(it.amount)}
                        </td>
                      </tr>
                    ))
          ...
          <div className="mt-6 border-t border-[#e5e7eb] pt-5 flex justify-end">
            <div className="w-full sm:w-[300px] space-y-2">
              ...
              <div className="flex justify-between text-[16px] sm:text-[18px] font-bold text-[#111827] pt-2 border-t border-[#e5e7eb]">
                <span>Total</span>
                <span>{fmt(total)}</span>
              </div>
              <div className="flex justify-between text-[13px] sm:text-[14px] text-[#10b981]">
                <span>Paid</span>
                <span>{fmt(paid)}</span>
              </div>
              <div
                className={`flex justify-between text-[14px] sm:text-[15px] font-semibold ${
          ...
          {bill.notes && (
            <div className="mt-6 rounded-[12px] bg-[#f9fafb] px-4 py-3 text-[12px] sm:text-[13px] text-[#374151]">
        ...
        {/* Footer */}
        <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 px-4 sm:px-8 py-4 border-t border-[#e5e7eb] bg-[#f9fafb] sticky bottom-0">
          <div className="flex gap-3">
            <button
              onClick={handlePrint}
              className="h-[40px] sm:h-[42px] px-4 sm:px-5 rounded-[12px] bg-[#6B21A8] text-white text-[13px] sm:text-[14px] font-semibold flex items-center justify-center gap-2 hover:bg-[#581c87] transition"
            >
            ...
            {onEdit && (
              <button
                onClick={onEdit}
                className="h-[40px] sm:h-[42px] px-4 sm:px-5 rounded-[12px] border border-[#d1d5db] bg-white text-[#374151] text-[13px] sm:text-[14px] font-semibold flex items-center justify-center gap-2 hover:bg-[#f9fafb] transition"
              >
                <Pencil size={16} /> Edit
              </button>
            )}
          ...
          <button
            onClick={onClose}
            className="h-[40px] sm:h-[42px] rounded-[12px] border border-[#d1d5db] px-5 text-[13px] sm:text-[14px] font-medium text-[#374151]"
          >
            Close
          </button>
        </div>
...
function Th({ children, align = "left", className: extra = "" }) {
  return (
    <th
      className={`px-3 sm:px-4 py-3 text-${align} text-[11px] sm:text-[12px] font-semibold text-[#6b7280] uppercase tracking-wide ${extra}`}
    >
      {children}
    </th>
  );
...
function Row({ label, value }) {
  return (
    <div className="flex justify-between text-[13px] sm:text-[14px] text-[#6b7280]">
      <span>{label}</span>
      <span>{value}</span>
    </div>
  );
