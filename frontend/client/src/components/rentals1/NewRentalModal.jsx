      ...
      <div className="w-full max-w-[820px] h-[90vh] rounded-[14px] sm:rounded-[18px] bg-white shadow-2xl flex flex-col overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-4 sm:px-6 md:px-8 py-4 sm:py-6">
          ...
          {formData.items.length === 0 ? (
            <p className="mt-4 text-[14px] sm:text-[16px] text-[#6b7280]">No items added yet</p>
          ) : (
            <div className="mt-3 sm:mt-4 space-y-3 sm:space-y-4">
              {formData.items.map((item, index) => {
                const selectedInventoryItem = inventoryItems.find((inv) => String(inv.id) === String(item.itemId));
                const maxAvailable = selectedInventoryItem ? Number(selectedInventoryItem.availableQuantity || 1) : 1;
                const rowTotal = Number(item.quantity || 0) * Number(item.days || 0) * Number(item.rate || 0);

                return (
                  <div key={`${item.itemId}-${index}`} className="rounded-[12px] sm:rounded-[14px] border border-[#e5e7eb] p-3 sm:p-4">
                    {/* Mobile */}
                    <div className="block sm:hidden space-y-3">
                      <div>
                        <label className="mb-1 block text-[12px] font-medium text-[#111827]">Item</label>
                        <select value={item.itemId} onChange={(e) => handleSelectInventoryItem(index, e.target.value)}
                          className="h-[40px] w-full rounded-[10px] border border-[#d1d5db] px-3 outline-none text-[13px]">
                          <option value="">Select item</option>
                          {availableInventoryItems.map((invItem) => (
                            <option key={invItem.id} value={invItem.id}>{invItem.name} ({invItem.sku})</option>
                          ))}
                        </select>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        <div>
                          ...
                          <input type="number" min="1" max={maxAvailable} value={item.quantity}
                            ...
                            className="h-[40px] w-full rounded-[10px] border border-[#d1d5db] px-2 outline-none text-[13px]" />
                        </div>
                        <div>
                          <label className="mb-1 block text-[12px] font-medium text-[#111827]">Days</label>
                          <input type="number" min="1" value={item.days}
                            onChange={(e) => handleItemFieldChange(index, "days", e.target.value)}
                            className="h-[40px] w-full rounded-[10px] border border-[#d1d5db] px-2 outline-none text-[13px]" />
                        </div>
                        <div>
                          <label className="mb-1 block text-[12px] font-medium text-[#111827]">Subtotal</label>
                          <div className="h-[40px] rounded-[10px] border border-[#e5e7eb] bg-[#f9fafb] px-2 flex items-center text-[13px] font-medium">
                            ₹{rowTotal.toFixed(2)}
                          </div>
                        </div>
                      </div>
                      <div className="flex justify-end">
                        <button type="button" onClick={() => handleRemoveItem(index)}
                          className="h-[36px] w-[36px] rounded-[10px] border border-[#fecaca] text-red-500 flex items-center justify-center">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
          ...
          <div className="mt-6 sm:mt-8 rounded-[12px] sm:rounded-[16px] border border-[#e5e7eb] p-3 sm:p-4 md:p-5">
            ...
            <div className="mt-4 sm:mt-5 space-y-1.5 sm:space-y-2 text-[14px] sm:text-[16px] text-[#111827]">
              ...
              <div className="flex justify-between pt-2 sm:pt-3 text-[16px] sm:text-[18px] md:text-[20px] font-bold">
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 sm:gap-3 px-4 sm:px-6 md:px-8 py-3 sm:py-4 border-t border-gray-200 flex-shrink-0">
          ...
          <button type="button" onClick={handleSubmit} disabled={loading}
            className="h-[42px] sm:h-[44px] rounded-[12px] bg-[#8fb1f8] px-5 text-[14px] font-semibold text-white disabled:opacity-60 w-full sm:w-auto">
